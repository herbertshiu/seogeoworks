...
const researchPosts = [
  ...
  { category: "Expert insight", type: "Briefing", title: "Why third-party proof is becoming part of your owned strategy", excerpt: "The distribution moves that help a brand become recognizable beyond its own domain.", date: "Aug 17, 2026", read: "7 min", tone: "research-violet", href: "/resources/geo-readiness-checklist.html" },
];
...
const uiCopy = {
  en: { navBriefings: "Briefings", navCompare: "SEO vs GEO", navCase: "Case study", navRoi: "ROI calculator", navAudit: "GEO audit", navVoices: "Voices", navNotes: "Field notes", navResearch: "Research desk", navAbout: "About", light: "Light", dark: "Dark", getSignal: "Get the signal", youIn: "You’re in", eyebrow: "The people & patterns behind modern search", heroTitle: "Be findable.", heroStay: "Stay human.", heroBody: "seogeoworks is an independent field guide to SEO, GEO, and the operators reshaping how brands get found, understood, and remembered.", explore: "Explore the latest", meet: "Meet the desk", compareTitle: "SEO gets you", compareEm: "seen. GEO gets you chosen.", researchTitle: "Latest thinking.", researchEm: "Sharper signals.", auditTitle: "Turn a vague", auditEm: "gap into a plan.", newsletterTitle: "Good thinking,", newsletterEm: "once a week.", newsletterBody: "One thoughtful dispatch on SEO, GEO, and building a brand that machines can understand — and people can feel.", email: "Your email address", welcome: "Welcome to seogeoworks.", checkReadiness: "Check your readiness" },
  zhHans: { navBriefings: "研究简报", navCompare: "SEO 对比 GEO", navCase: "案例研究", navRoi: "ROI 计算器", navAudit: "GEO 审计", navVoices: "专家声音", navNotes: "现场笔记", navResearch: "研究中心", navAbout: "关于", light: "浅色", dark: "深色", getSignal: "订阅信号", youIn: "已订阅", eyebrow: "现代搜索背后的人与规律", heroTitle: "让人找到。", heroStay: "保持真实。", heroBody: "seogeoworks 是一本独立的 SEO 与 GEO 现场指南，记录正在重塑品牌被发现、理解与记住方式的实践者。", explore: "探索最新内容", meet: "认识专家", compareTitle: "SEO 让你", compareEm: "被看见，GEO 让你被选择。", researchTitle: "最新洞见。", researchEm: "更清晰的信号。", auditTitle: "把模糊的", auditEm: "问题变成计划。", newsletterTitle: "好思考，", newsletterEm: "每周一次。", newsletterBody: "每周一份关于 SEO、GEO，以及如何让机器理解、让人感受到品牌的精选简报。", email: "你的邮箱地址", welcome: "欢迎来到 seogeoworks。", checkReadiness: "检查你的准备度" },
  zhHant: { navBriefings: "研究簡報", navCompare: "SEO 對比 GEO", navCase: "案例研究", navRoi: "ROI 計算器", navAudit: "GEO 審計", navVoices: "專家聲音", navNotes: "現場筆記", navResearch: "研究中心", navAbout: "關於", light: "淺色", dark: "深色", getSignal: "訂閱訊號", youIn: "已訂閱", eyebrow: "現代搜尋背後的人與規律", heroTitle: "讓人找到。", heroStay: "保持真實。", heroBody: "seogeoworks 是一本獨立的 SEO 與 GEO 現場指南，記錄正在重塑品牌被發現、理解與記住方式的實踐者。", explore: "探索最新內容", meet: "認識專家", compareTitle: "SEO 讓你", compareEm: "被看見，GEO 讓你被選擇。", researchTitle: "最新洞見。", researchEm: "更清晰的訊號。", auditTitle: "把模糊的", auditEm: "問題變成計劃。", newsletterTitle: "好思考，", newsletterEm: "每週一次。", newsletterBody: "每週一份關於 SEO、GEO，以及如何讓機器理解、讓人感受到品牌的精選簡報。", email: "你的電郵地址", welcome: "歡迎來到 seogeoworks。", checkReadiness: "檢查你的準備度" },
} as const;
...
function App() {
  ...
  const [theme, setTheme] = useState<"dark" | "light">(() => (localStorage.getItem("seogeoworks-theme") as "dark" | "light") || "dark");
  const [language, setLanguage] = useState<keyof typeof uiCopy>(() => (localStorage.getItem("seogeoworks-language") as keyof typeof uiCopy) || "en");
  const copy = uiCopy[language];

  useEffect(() => {
    document.documentElement.dataset.theme = theme;
    localStorage.setItem("seogeoworks-theme", theme);
  }, [theme]);
  useEffect(() => {
    document.documentElement.lang = language === "zhHans" ? "zh-CN" : language === "zhHant" ? "zh-HK" : "en";
    localStorage.setItem("seogeoworks-language", language);
  }, [language]);
  ...
  const copyAuditSummary = async () => { await navigator.clipboard?.writeText(`seogeoworks GEO audit: ${auditScore}% — ${auditLabel}. ${auditComplete}/${allAuditChecks.length} checks complete.`); setAuditCopied(true); setTimeout(() => setAuditCopied(false), 1800); };
  ...
  const navItems: Array<{ label: string; href: string; scrollId?: string }> = [
    { label: copy.navBriefings, href: "https://www.seogeoworks.hk/#briefings", scrollId: "briefings" },
    { label: copy.navCompare, href: "https://www.seogeoworks.hk/resources/seo-vs-geo.html" },
    { label: copy.navCase, href: "https://www.seogeoworks.hk/#case-study", scrollId: "case-study" },
    { label: copy.navRoi, href: "https://www.seogeoworks.hk/#roi", scrollId: "roi" },
    { label: copy.navAudit, href: "https://www.seogeoworks.hk/resources/geo-readiness-checklist.html" },
    { label: copy.navVoices, href: "https://www.seogeoworks.hk/#voices", scrollId: "voices" },
    { label: copy.navNotes, href: "https://www.seogeoworks.hk/articles/", scrollId: undefined },
    { label: copy.navResearch, href: "https://www.seogeoworks.hk/resources/" },
    { label: copy.navAbout, href: "https://www.seogeoworks.hk/about/" },
  ];
  ...
  return (
    <div className="site-shell">
      ...
      <header className="site-header">
        ...
        <a className="wordmark" href="https://www.seogeoworks.hk/" onClick={(event) => handleNavClick(event, "top")}><span className="wordmark-dot" />seogeo<span>works</span></a>
        <nav className={menuOpen ? "nav-links open" : "nav-links"}>
          {navItems.map((item) => (
            <a
              ...
              href={item.href}
              onClick={(event) => {
                if (item.scrollId) handleNavClick(event, item.scrollId);
      ...
      <main id="top">
        <section className="hero section-grid">
          <div className="hero-copy">
            ...
            <h1>{language === "en" ? <>Be <em>findable.</em><br />Stay human.</> : <>{copy.heroTitle}<br /><em>{copy.heroStay}</em></>}</h1>
            <p className="hero-lede">{copy.heroBody}</p>
            <div className="hero-actions"><a className="button-primary" href="/articles/">{copy.explore} <ChevronRight size={17} /></a><a className="text-button" href="/resources/">{copy.meet} <ArrowUpRight size={16} /></a></div>
            <div className="hero-proof"><div className="avatar-stack"><span>SL</span><span>OM</span><span>AK</span><b>+</b></div><span>{language === "en" ? <>Conversations with 42 practitioners<br /><strong>across search, content & AI</strong></> : <>与 42 位实践者对话<br /><strong>涵盖搜索、内容与 AI</strong></>}</span></div>
          </div>
        ...
        <section id="field-notes" className="section notes-section"><div className="section-heading compact"><div><div className="eyebrow"><span className="eyebrow-line" /> Dispatches</div><h2>Small notes.<br /><em>Sharp edges.</em></h2></div><button className="filter-button"><span>All topics</span><ChevronRight size={15} /></button></div><div className="notes-list">{notes.map((note, index) => <a className="note-row" href={note.href} key={note.title}><div className={`note-index ${note.color}`}>0{index + 1}</div><div className="note-main"><span className="note-tag">{note.tag}</span><h3>{note.title}</h3></div><div className="note-meta"><span>{note.date}</span><span><Clock3 size={14} /> {note.time}</span></div><span className="note-arrow" aria-hidden="true"><ArrowUpRight size={18} /></span></a>)}</div></section>

        <section id="research-desk" className="research-section"><div className="section-heading compact"><div><div className="eyebrow"><span className="eyebrow-line" /> Research desk</div><h2>{language === "en" ? <>Latest thinking.<br /><em>Sharper signals.</em></> : <>{copy.researchTitle}<br /><em>{copy.researchEm}</em></>}</h2></div><p className="section-intro">A living library for GEO research, operator conversations, and the patterns worth carrying into your next brief.</p></div><div className="research-toolbar"><span>Browse the desk</span><div>{["All", "GEO research", "Expert insight", "SEO / GEO"].map((filter) => <button key={filter} className={researchFilter === filter ? "active" : ""} onClick={() => setResearchFilter(filter)}>{filter}</button>)}</div></div><div className="research-grid">{filteredResearch.map((post, index) => <a className={`research-card ${post.tone}`} href={post.href} key={post.title}><div className="research-card-top"><span>{post.type}</span><b>0{index + 1}</b></div><div className="research-card-art"><div className="research-art-line line-a" /><div className="research-art-line line-b" /><div className="research-art-dot" /></div><div className="research-card-copy"><span>{post.category} · {post.date}</span><h3>{post.title}</h3><p>{post.excerpt}</p><span className="read-link">Open piece <ArrowUpRight size={16} /></span></div></a>)}</div><div className="research-footer"><span><Send size={15} /> New: the research desk is updated with every issue.</span><button className="text-button" onClick={() => scrollTo("about")}>Get the weekly signal <ArrowUpRight size={16} /></button></div></section>
      ...
      <footer className="site-footer"><a className="wordmark footer-mark" href="https://www.seogeoworks.hk/" onClick={(event) => handleNavClick(event, "top")}><span className="wordmark-dot" />seogeo<span>works</span></a><span>© 2026 seogeoworks. Made for the curious.</span><div className="footer-links"><a href="https://www.seogeoworks.hk/about/">About</a><a href="https://www.seogeoworks.hk/resources/">Resources</a><a href="https://www.seogeoworks.hk/articles/">Field guides</a><a href="https://www.seogeoworks.hk/contact/">Contact</a><a href="https://www.seogeoconsulting.hk/" rel="noopener noreferrer">SEO / GEO Consulting</a><a href="https://www.seogeoconsulting.hk/authority" rel="noopener noreferrer">Authority scorecard</a><a href="https://www.myairanking.hk/" rel="noopener noreferrer">AI Ranking Guide</a><a href="https://www.mysearchvisibility.hk/" rel="noopener noreferrer">My Search / Visibility</a><a href="https://itehk.com.hk/" rel="noopener noreferrer">Itehk</a><a href="https://www.seogeoworks.hk/llms.txt">llms.txt</a><a href="https://www.seogeoworks.hk/sitemap.xml">Sitemap</a></div></footer>
    </div>
  );
}
